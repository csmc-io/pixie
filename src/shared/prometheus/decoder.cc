...
std::string FetchPrometheusMetrics(const std::string& url) {
  ...
  if (r.status_code == 0) {
    LOG(ERROR) << absl::Substitute("Failed to fetch prometheus metrics from $0 error=$1", url,
                                   r.error.message);
  } else if (r.status_code >= 400) {
    LOG(ERROR) << absl::Substitute("Failed to fetch prometheus metrics from $0 status_code=$1", url,
                                   r.status_code);
  }
  return r.text;
}
