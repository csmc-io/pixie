...
void SocketTraceConnector::TransferDataImpl(ConnectorContext* ctx) {
  ...
  LOG(INFO) << absl::Substitute("Total buffer size of all active ConnTrackers: $0",
                                 total_conn_tracker_mem_usage_);
...
void SocketTraceConnector::AcceptDataEvent(std::unique_ptr<SocketDataEvent> event) {
  ...
  if (FLAGS_total_conn_tracker_mem_usage != 0 && total_conn_tracker_mem_usage_ > FLAGS_total_conn_tracker_mem_usage) {
    LOG(WARNING) << absl::Substitute(
        "Total buffer size of all active ConnTrackers $0 exceeds the limit $1. "
        "Dropping data event of size $2 for protocol $3",
        total_conn_tracker_mem_usage_, FLAGS_total_conn_tracker_mem_usage, event->msg.size(), event->attr.protocol);
...
void SocketTraceConnector::AcceptHTTP2Data(std::unique_ptr<HTTP2DataEvent> event) {
  if (FLAGS_total_conn_tracker_mem_usage != 0 && total_conn_tracker_mem_usage_ > FLAGS_total_conn_tracker_mem_usage) {
    LOG(WARNING) << absl::Substitute(
        ...
        total_conn_tracker_mem_usage_, FLAGS_total_conn_tracker_mem_usage, event->payload.size());
    stats_.Increment(StatKey::kDroppedSocketDataEventCount);
    return;
  }
  ConnTracker& tracker = GetOrCreateConnTracker(event->attr.conn_id);
